...
namespace vcdb.MySql.SchemaBuilding
{
    public class MySqlIndexRepository : IIndexRepository
    {
        ...
        {
            var sql = @"select index_name, index_type as type_desc, case non_unique when 0 then 1 else 0 end as is_unique, column_name
            ...
            return columnsInEachIndex.ToDictionary(
                ...
                group =>
                {
                    ...
                    return new IndexDetails
                    {
                        ...
                        Clustered = indexDetails.type_desc == "TODO", //Whats the name of a clustered index in MySql??
