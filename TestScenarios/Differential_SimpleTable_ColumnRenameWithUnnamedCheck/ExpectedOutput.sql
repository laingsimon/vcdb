...
GO
ALTER TABLE [dbo].[Person]
ADD CONSTRAINT [CK__Person__FullName__0]
CHECK (len([FullName])>3)
GO
DECLARE @newName VARCHAR(1024)
SELECT @newName = 'CK__Person__FullName__' + FORMAT(chk.OBJECT_ID, 'X')
...
AND chk.name = 'CK__Person__FullName__0'

EXEC sp_rename 
    @objname = 'CK__Person__FullName__0', 
    @newname = @newName, 
    @objtype = 'OBJECT'
GO